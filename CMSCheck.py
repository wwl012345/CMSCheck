...
if __name__ == "__main__":
    ...
    用法：
    单个域名进行检测：python3 TideFinger.py -u http://www.example.com [-p 1] [-m 50] [-t 5] [-d 1]
    多个域名进行检测：python3 TideFinger.py -f url.txt [-p 1] [-m 50] [-t 5] [-d 1] 
    '''
    if len(sys.argv) < 2 or sys.argv[1] == '-h':
        ...
        try:
            ...
            request_timeout = 5
            dir_mode = 0
            options,args = getopt.getopt(sys.argv[1:], "u:f:p:m:t:d:")
            ip = ''
            m_count = 100
            target_url = []
            ping = True
            ...
            for i in target_url:
                ...
                if re.match(r'^https?:/{2}\w.+$', target):
                    ...
                    for x in fofa_finger:
                        banner.append(x)
                    try:
                        Wappalyzer = useWappalyzer(target)
                    ...
                    try:
                        update = False
                        webanalyzer_banner = webanalyzer.check(target, update)
                    ...
                    if not cms_name_flag:
                        if dir_mode == 1:
                            cms_name_tmp = finger_query(target)
                            if cms_name_tmp:
